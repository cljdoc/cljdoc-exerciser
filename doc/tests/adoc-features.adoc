...
In SECURE safe mode, I expect JavaScript not to function:
++++
...
Here we tell JavaScript to popup and alert:
...
Asciidoctor explicitly supports videos with a variety of options.
...
Asciidoctor also allows audio play.

audio::https://upload.wikimedia.org/wikipedia/commons/3/3c/01_-_Vivaldi_Spring_mvt_1_Allegro_-_John_Harrison_violin.ogg[]

=== Stem

Stem supports mathematical equations via MathJax, which is neat, but we don't support it at this time.
